  ...
  module Api
    module V1
      ...
      class EpisodeContract < Dry::Validation::Contract
        ...
        rule('clinical_info.primary_su_age_at_first_use', 'client.dob', :admission_date) do
          if key && (values.dig(:clinical_info, :primary_su_age_at_first_use) && values.dig(:client, :dob) && values[:admission_date])
            now = values[:admission_date]
            dob = values.dig(:client, :dob)
            admission_age = now.year - dob.year - (now.month > dob.month || (now.month == dob.month && now.day >= dob.day) ? 0 : 1)
            key(:primary_su_age_at_first_use).failure(:primary_su_age_at_first_use_greater_than_admission_age) if values.dig(:clinical_info, :primary_su_age_at_first_use).to_i > admission_age
          end
        end
